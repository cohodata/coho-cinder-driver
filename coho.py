...
class Client(object):
    ...
    def unpack_replyheader(self):
        ...
        if stat == MSG_DENIED:
            ...
            raise exception.VolumeDriverException(_('MSG_DENIED: %r') % (stat,))
        ...
        if stat == PROG_UNAVAIL:
            raise exception.VolumeDriverException(_('call failed: PROG_UNAVAIL'))
        ...
        if stat == PROC_UNAVAIL:
            raise exception.VolumeDriverException(_('call failed: PROC_UNAVAIL'))
        if stat == GARBAGE_ARGS:
            raise exception.VolumeDriverException(_('call failed: GARBAGE_ARGS'))
        if stat != SUCCESS:
            raise exception.VolumeDriverException(_('call failed: %r') % (stat,))
        return xid, verf
    ...
    def _call(self, proc, args):
        ...
        if res != SUCCESS:
            raise exception.VolumeDriverException(os.strerror(res))
